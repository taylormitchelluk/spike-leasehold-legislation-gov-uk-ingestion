...
Reshape the project into a pnpm monorepo orchestrated by Turbo, with a `justfile` for common developer commands.
...
Use Turbo for package task orchestration and `just` as the human-friendly command entrypoint.
...
- Add a root `justfile` that wraps common pnpm/Turbo commands.
...
11. Update README examples to prefer `just` commands while retaining the underlying pnpm commands where useful.
...
- A root `justfile` provides the common developer command surface.
- `just dev` starts the ingestion Worker locally.
- `just typecheck` runs the Worker typecheck through Turbo.
